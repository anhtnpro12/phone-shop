    ...
    <form action="{{ route('products.update', ['product' => $product->id]) }}" method="post" enctype="multipart/form-data" style="width: 50%;">
        ...
        <div class="mb-3">
            ...
            <select id="trending" name="trending" class="selectpicker"
                    data-live-search="true" data-width="100%"
                    data-style="border" data-size="5">
                    <option value="1" {{ $product->trending==1?'selected':'' }} data-content='<span class="badge bg-danger">Trending</span>'>Trending</option>
                    <option value="2" {{ $product->trending==2?'selected':'' }} data-content='<span class="badge bg-secondary">Normal</span>'>Normal</option>
            </select>
        </div>
